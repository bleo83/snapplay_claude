...
class JdbcExperienceRepositoryTest {
    ...
    fun setup() {
        listOf(
            "DELETE FROM experience_versions", "DELETE FROM experiences",
            "DELETE FROM catalog_products", "DELETE FROM commercial_contracts",
            "DELETE FROM contract_versions", "DELETE FROM connections",
            "DELETE FROM content_contexts", "DELETE FROM channels",
            "DELETE FROM data_sharing_policies", "DELETE FROM organizations",
        ...
        jdbc.update("INSERT INTO organizations (id, legal_name, display_name, organization_type, country, default_currency, timezone) VALUES (?, 'Disney', 'Disney', 'CONTENT_PROVIDER', 'AR', 'ARS', 'America/Argentina/Buenos_Aires')", orgId)
        jdbc.update("INSERT INTO organizations (id, legal_name, display_name, organization_type, country, default_currency, timezone) VALUES (?, 'Rappi', 'Rappi', 'COMMERCE_PROVIDER', 'AR', 'ARS', 'America/Argentina/Buenos_Aires')", commerceOrgId)
        jdbc.update("INSERT INTO channels (id, organization_id, channel_key, display_name) VALUES (?, ?, 'disney_plus', 'Disney+')", channelId, orgId)
        jdbc.update("INSERT INTO content_contexts (id, organization_id, channel_id, external_ref, context_type, title) VALUES (?, ?, ?, 'toy_story', 'MOVIE', 'Toy Story')", contextId, orgId, channelId)
        jdbc.update("INSERT INTO data_sharing_policies (id, owner_organization_id, name, mode) VALUES (?, ?, 'default', 'BILLING_ONLY')", policyId, orgId)
        jdbc.update("INSERT INTO connections (id, name, content_organization_id, commerce_organization_id, connector_key, environment, territories, capabilities, data_sharing_policy_id, status) VALUES (?, 'Disney x Rappi', ?, ?, 'rappi', 'SANDBOX', '{AR}', '{}', ?, 'ACTIVE')", connectionId, orgId, commerceOrgId, policyId)
        jdbc.update("INSERT INTO commercial_contracts (id, name, publisher_organization_id, commerce_organization_id, territories, status) VALUES (?, 'Pilot', ?, ?, '{AR}', 'ACTIVE')", contractId, orgId, commerceOrgId)
    }
    ...
    fun `create inserts experience and version, returns domain object`() {
        val input = CreateExperienceInput(
    ...
    fun `findAll returns created experience with correct joins`() {
        repository.create(orgId, UUID.randomUUID(), CreateExperienceInput("Night", "Toy Story", HandoffMode.STORE_DEEPLINK, 0, Instant.now(), null))
