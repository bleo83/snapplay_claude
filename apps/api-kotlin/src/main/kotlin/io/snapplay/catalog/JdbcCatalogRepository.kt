...
class JdbcCatalogRepository(private val jdbc: JdbcTemplate) : CatalogRepository {
    ...
    private val rowMapper = RowMapper { rs: ResultSet, _ ->
        CatalogProduct(
            ...
            categories = (rs.getArray("categories")?.array as? kotlin.Array<*>)
                ?.filterIsInstance<String>() ?: emptyList(),
            ageRestricted = rs.getBoolean("age_restricted"),
            referencePriceMinor = rs.getLong("reference_price_minor")
                .takeIf { !rs.wasNull() },
    ...
    override fun findProducts(organizationId: UUID, filters: ProductFilters): List<CatalogProduct> {
        val sql = buildString {
            append(
                """
